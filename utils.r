...
process_gbif_data <- function(species_name, lon_range = c(-85,-32), lat_range = c(-58, 15),
  ...
  species_filtered <- species_coords %>%
    ...
    tidyr::drop_na(decimalLongitude, decimalLatitude, year, day, month)
  ...
  write.csv(unique_species, file.path(output_dir, output_file), row.names = FALSE)
  sp::coordinates(unique_species) <- ~ decimalLongitude + decimalLatitude
...
generate_model <- function(data_path,
  ...
  csv_full_path <- file.path(data_path, csv_filename)
  df <- read.csv(csv_full_path, sep = ",", header = TRUE)

  inferred <- infer_predictors_from_csv(csv_full_path,
  ...
  sdmDataObj <- sdm::sdmData(form, train = df)

  # aqui usa o n_reps
  modelo <- sdm::sdm(form, sdmDataObj,
                methods     = methods,
                replication = c("boot"),
                n           = n_reps)

  sdm::write.sdm(modelo, file.path(output_path, model_name), overwrite = TRUE)
  return(modelo)
}
...
generate_vif <- function(csv_path,
  ...
  sp::coordinates(df) <- stats::as.formula(paste0("~", lon_col, "+", lat_col))
  ext <- raster::extent(df)
  r_template <- raster::raster(ext, res = res)

  # Rasterizar cada variável preditora e empilhar
  r_list <- list()
  for (v in predictor_names) {
    vals <- raster::rasterize(df, r_template, field = v, fun = mean)
    r_list[[v]] <- vals
  }
  env_stack <- raster::stack(r_list)
  ...
  message("VIFSTEP")
  print(usdm::vifstep(env_stack, th = vif_threshold))
  message("VIFCOR")
  print(usdm::vifcor(env_stack, th = vif_threshold))
}
